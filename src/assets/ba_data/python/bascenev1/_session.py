...
from bascenev1._powerup import get_default_powerup_distribution as gdpd
                ...
                try:
                    ...
                    if babase.app.classic and babase.app.classic.server:
                        plrdatapath = gdata.getpath(
                            'gstats/'
                            + (sessionplayer.get_v1_account_id() or 'anon')
                        )
                        plrdata = gdata.load(plrdatapath) or {}
                        plrdata.setdefault('lang', None)
                        plrdata.setdefault('lastlogin', 0)
                        plrdata.setdefault('streak', 0)
                        plrdata.setdefault('items', [])
                        plrdata.setdefault('spoints', 0)
                        cid = sessionplayer.inputdevice.client_id
                        inform('initUseHelp', (1, 0, 0.9), cid, plrdata['lang'])
                        if plrdata['lastlogin'] + 86400 < time.time():
                            if plrdata['lastlogin'] + 172800 < time.time():
                                plrdata['streak'] = 0
                            dlrmax = 25
                            if any(o.split('␟')[0] == 'vip@other'
                                   for o in plrdata['items']):
                                dlrmax = 50
                                powerupdist: list[str] = []
                                for powerup, freq in gdpd():
                                    for _i in range(int(freq)):
                                        powerupdist.append(powerup)
                                pu = random.choice(powerupdist) + '@powerup'
                                plrdata['items'].append('wheel@other')
                                plrdata['items'].append(pu)
                                inform('gotItem', 'success', cid,
                                       plrdata['lang'], ['wheel@other'])
                                inform('gotItem', 'success', cid,
                                       plrdata['lang'], [pu])
                            dlr = min(15 + plrdata['streak'], dlrmax)
                            plrdata['spoints'] += dlr
                            plrdata['streak'] += 1
                            plrdata['lastlogin'] = time.time()
                            inform('dailyLoginReward', 'success', cid,
                                   plrdata['lang'],
                                   [str(dlr), str(min(dlr + 1, dlrmax))])
                        plrdata['un'] = (
                            sessionplayer.inputdevice.get_v1_account_name(True)
                            if sessionplayer.get_v1_account_id()
                            else babase.charstr(babase.SpecialChar.TEST_ACCOUNT)
                                 + 'ANON'
                        )
