...
class ActorMode:
    """
    Base class for actor modes.

    Category: **Gameplay Classes**

    Actor modes can be used to let actors know that they should behave in a
    specific way; In order to use them, do Actor.mode = ActorModeClass.
    Note that the base Actor class does not handle any actor modes no matter how
    simple or complicated they are, instead each Actor should incorporate actor
    modes into their code if needed.

    This specific class should only be used for type checking.
    """


class ProjectileActorMode(ActorMode):
    """
    Send a bs.HitMessage to anything you touch
    and die once you deliver your bs.HitMessage.
    """


class SmashActorMode(ActorMode):
    """
    Save the amount of damage you receive as a percentage,
    handle impulse messages more extremely based on the damage percentage,
    and explode once you die.
    """
