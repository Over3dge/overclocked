...
class ChosenOneGame(bs.TeamGameActivity[Player, Team]):
    ...
    def create_team(self, sessionteam: bs.SessionTeam) -> Team:
        return Team(
            time_remaining=self._lowest
            if self._is_on_overtime
            else self._chosen_one_time
        )
    ...
    def overtime_description(self) -> str:
        ...
        ) + str(self._lowest) + ' seconds remaining'

    def overtime(self) -> None:
        for team in self.teams:
            self._lowest = min(self._lowest, team.time_remaining)
        for team in self.teams:
            team.time_remaining = self._lowest
        self._update_scoreboard()
