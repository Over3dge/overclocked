...
class Icon(bs.Actor):
    ...
    def update_for_lives(self) -> None:
        ...
        self._name_text.opacity = 0.2 if lives == 0 else 1.0
        assert self.node
        self.node.color = (0.7, 0.3, 0.3) if lives == 0 else (1.0, 1.0, 1.0)
        self.node.opacity = 0.2 if lives == 0 else 1.0
...
class EliminationGame(bs.TeamGameActivity[Player, Team]):
    ...
    def _is_meeting_overtime_conditions(self) -> bool:
        return True if self._allow_overtime else False

    @property
    def overtime_description(self) -> str:
        return 'Everyone including eliminated folks gets a final life'

    def overtime(self) -> None:
        if self._solo_mode:
            for team in self.teams:
                team.spawn_order = team.spawn_order[:1]
                for player in team.players:
                    if player in team.spawn_order:
                        player.lives = 1
                    else:
                        player.lives = 0
            self._update_icons()
