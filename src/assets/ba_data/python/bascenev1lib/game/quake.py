...
class QuakeBallFactory(object):
    ...
    def shot(self, spaz):
        ...
        if time - self.last_shot > 0.6:
            ...
            mag = 10.0 / bs.Vec3(*direction).length()
    ...
    def __init__(
        ...
        self.node = bs.newnode(
            ...
            attrs={
                ...
                'body_scale': 0.8,
                'materials': [shared.object_material, b_shared.ball_material],
            },
        )

        light_node = bs.newnode(
            'light',
            owner=self.node,
            attrs={
                'position': position,
        ...
        self.node.connectattr('position', light_node, 'position')
        self.emit_time = bs.Timer(0.015, bs.WeakCall(self.emit), repeat=True)
        self.life_time = bs.Timer(
            5.0, bs.WeakCall(self.handlemessage, bs.DieMessage())
        )
    ...
    def handlemessage(self, m):
        if isinstance(m, TouchedToAnything):
            ...
            if node is not None and node.exists():
                ...
                node.handlemessage(
                    bs.HitMessage(
                        pos=t,
                        velocity=v,
                        magnitude=bs.Vec3(*v).length() * 40,
                        velocity_magnitude=bs.Vec3(*v).length() * 40,
                        radius=0,
                        srcnode=self.node,
            ...
            if self.node.exists():
                ...
                bs.getsound(
                    random.choice(['impactHard', 'impactHard2', 'impactHard3'])
                ).play(),
            ...
            if (
                ...
                and node.exists()
                and node != self.owner
                and node.getdelegate(object).team != self.owner.team
            ):
...
class QuakeGame(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        self.drop_shield_timer: bs.Timer | None = None
        self._checker: bs.Timer | None = None
    ...
    def on_team_join(self, team: Team) -> None:
        if self._is_on_overtime:
            team.score = self._score_to_win - 1
        if self.has_begun():
    ...
    def on_begin(self) -> None:
        ...
        self.drop_shield()
        self.drop_shield_timer = bs.Timer(
            8.001, bs.WeakCall(self.drop_shield), repeat=True
        )

        shared = SharedObjects.get()
        smap = self.map.getname()
        if self.settings['Obstacles']:
            count = self.settings['Obstacle Count']
            for i in range(count):
                if smap == 'Football Stadium':
                    radius = (
                ...
                Obstacle(
                    ...
                    shape=int(self.settings['Obstacles Shape']),
                ).autoretain()

        if len(self.teams) > 0:
            self._score_to_win = self.settings['Kills to Win Per Player'] * max(
    ...
    def drop_shield(self):
        ...
        def check_exists():
            if p is None or not p.node.exists():
                del_checker()

        self._checker = bs.Timer(0.1, bs.Call(check_exists), repeat=True)

        def del_checker():
            if self._checker is not None:
                self._checker = None

        bs.timer(6.9, bs.Call(del_checker))
    ...
    def _is_meeting_overtime_conditions(self) -> bool:
        return (
            True if self._allow_overtime and len(self.players) != 0 else False
        )
