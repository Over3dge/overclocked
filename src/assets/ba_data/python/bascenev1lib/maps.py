...
#
# This file also incorporates some work that is ALL RIGHTS RESERVED.
#
"""Standard maps."""
...
class SoccerStadium(bs.Map):
    """original stadium i stole from zacker_tz"""
    ...
    def on_preload(cls) -> Any:
        data: dict[str, Any] = {
            'meshes': (bs.getmesh('hockeyStadiumOuter'),
                       bs.getmesh('hockeyStadiumInner'),
                       bs.getmesh('hockeyStadiumStands')),
            'vr_fill_mesh': bs.getmesh('footballStadiumVRFill'),
            'collision_mesh': bs.getcollisionmesh('hockeyStadiumCollide'),
    ...
    def __init__(self) -> None:
        ...
        self.node = bs.newnode('terrain', delegate=self,
                               attrs={'mesh': self.preloaddata['meshes'][0],
                                      'collision_mesh': self.preloaddata[
                                          'collision_mesh'],
                                      'color_texture': self.preloaddata['tex'],
                                      'materials': [shared.footing_material]})
        bs.newnode('terrain',
                   attrs={'mesh': self.preloaddata['vr_fill_mesh'],
                          'vr_only': True, 'lighting': False,
                          'background': True,
                          'color_texture': self.preloaddata['stands_tex']})
        self.floor = bs.newnode('terrain',
                                attrs={'mesh': self.preloaddata['meshes'][1],
                                       'color_texture': self.preloaddata['tex'],
                                       'color': (0.1, 0.9, 0.2),
                                       'opacity': 0.92,
                                       'opacity_in_low_or_medium_quality': 1.0,
        ...
        self.stands = bs.newnode('terrain',
                                 attrs={'mesh': self.preloaddata['meshes'][2],
                                        'visible_in_reflections': False,
...
class OpenField(bs.Map):
    """Open Field map by Freakyy"""

    from bascenev1lib.mapdata import open_field as defs
    name = "Open Field"
    ...
    def __init__(self) -> None:
        ...
        y = -1
        while y > -11:
            self.zonebs.append(bs.newnode('locator',
                                          attrs={'shape': 'circle',
                                                 'position': (y, 0.01, 4),
                                                 'color': (0, 0, 1),
                                                 'opacity': 1,
            ...
            self.zonebs.append(bs.newnode('locator',
                                          attrs={'shape': 'circle',
                                                 ...
                                                 'additive': False,
                                                 'size': [0.40]}))
            self.zoners.append(bs.newnode('locator',
                                          attrs={'shape': 'circle',
                                                 'position': (-y, 0.01, 4),
                                                 'color': (1, 0, 0),
                                                 'opacity': 1,
...
class ClosedArena(bs.Map):
    """Closed Arena map by Freakyy"""

    from bascenev1lib.mapdata import closed_arena as defs
    name = 'Closed Arena'
