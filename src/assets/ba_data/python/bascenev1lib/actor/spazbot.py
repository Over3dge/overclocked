...
class SpazBot(Spaz):
    ...
    accept_powerups = False

    def __init__(self, team: bs.Team | None = None) -> None:
        ...
        super().__init__(
            ...
            can_accept_powerups=self.accept_powerups,
        )
        ...
        self.team = team
        self.botset = SpazBotSet(team) if self.accept_powerups else None
...
class EnemyBot(SpazBot):
    ...
    charge_dist_max = 5
    run = True
    accept_powerups = True
    points_mult = 5

    def __init__(self, team: bs.Team | None = None) -> None:
        super().__init__(team)
        self.handlemessage(bs.PowerupMessage('powerups'))
...
class SpazBotSet:
    ...
    def _update(self) -> None:
        ...
        for node in bs.getnodes():
            sbot = node.getdelegate(SpazBot)
            if sbot and sbot.team != self.team and sbot.is_alive():
                player_pts.append(
                    (
        ...
        for player in bs.getactivity().players:
            ...
            try:
                ...
                if player.team != self.team and player.is_alive():
