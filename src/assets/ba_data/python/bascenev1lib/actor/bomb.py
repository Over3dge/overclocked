...
class BombFactory:
    ...
    impulse_bomb_mesh: bs.Mesh
    """The bs.Mesh of an impulse-bomb."""

    dev_mesh: bs.Mesh
    """The bs.Mesh of a dev."""
    ...
    impulse_tex: bs.Texture
    """The bs.Texture for impulse bombs."""

    dev_tex: bs.Texture
    """The bs.Texture for devs."""
    ...
    def __init__(self) -> None:
        ...
        self.impulse_bomb_mesh = bs.getmesh('powerup')
        self.dev_mesh = bs.getmesh('shield')
        ...
        self.impulse_tex = bs.gettexture('achievementCrossHair')
        self.dev_tex = bs.gettexture('black')
        self.portal_tex = bs.gettexture('landMineLit')
        self.wonder_tex = bs.gettexture('wonderBombColor')
        ...
        self.bomb_scale_mult: float = 1.0
        self.blast_radius_mult: float = 1.0
        self.density_mult: float = 1.0
...
class Blast(bs.Actor):
    ...
    def __init__(
        ...
        explosion = bs.newnode(
            ...
            attrs={
                ...
                'velocity': evel,
                'radius': self.radius,
                'big': self.blast_type in ('tnt', 'impulse'),
            },
        )
        if self.blast_type in ('ice', 'icepact'):
            explosion.color = (0, 0.05, 0.4)

        bs.timer(1.0, explosion.delete)

        if self.blast_type not in ('ice', 'icepact', 'impulse', 'wonder'):
            bs.emitfx(
                position=position,
                velocity=velocity,
        ...
        if self.blast_type not in ('impulse', 'wonder'):
            bs.emitfx(
                position=position,
                velocity=velocity,
                ...
                emit_type='tendrils',
                tendril_type=(
                    'ice' if self.blast_type in ('ice', 'icepact') else 'smoke'
                ),
            )
        bs.emitfx(
            position=position,
        ...
        if self.blast_type in ('ice', 'impulse', 'wonder'):
        ...
        elif self.blast_type != 'icepact':
            # Regular or land mine bomb shrapnel.
        ...
        lcolor = (
            (0.6, 0.6, 1.0)
            if self.blast_type in ('ice', 'icepact', 'impulse')
            else (0.6, 1.0, 0.6)
            if self.blast_type == 'wonder'
            else (1.0, 0.3, 0.1)
        )
        light = bs.newnode(
            'light',
        ...
        if self.blast_type in ('ice', 'icepact', 'impulse'):
            scorch.color = (1.0, 1.0, 1.5)
        elif self.blast_type == 'wonder':
            scorch.color = (1.0, 1.5, 1.0)
    ...
    def handlemessage(self, msg: Any) -> Any:
        ...
        if isinstance(msg, bs.DieMessage):
            ...
            mag = 2000.0
            if self.blast_type in ('ice', 'icepact', 'wonder'):
                mag *= 0.5
            elif self.blast_type == 'land_mine':
                mag *= 2.5
            elif self.blast_type in ('tnt', 'impulse'):
                mag *= 2.0

            if self.blast_type == 'impulse':
                ...
                if spaz:
                    ...
                    spaz.last_player_attacked_by = bs.existing(
                        self._source_player
                    )
                    spaz.last_attacked_type = ('explosion', 'impulse')
                node.handlemessage(
                    'impulse',
                    nodepos[0],
                    nodepos[1],
                ...
                team = self._source_player.team if self._source_player else None
                spaz = node.getdelegate(Spaz)
                bomb = node.getdelegate(Bomb)
                powerupbox = node.getdelegate(PowerupBox)
                if spaz and spaz.team is team:
                    spaz.handlemessage(
                        bs.PowerupMessage(
                            random.choice(
                                ['health', 'powerups', 'shield', 'uno', 'inv']
                            ),
                        ),
                    )
                elif bomb:
                    ...
                    for i in range(4):
                        position = [
                            v + 0.4 if i == 0 else v for v in node.position
                        ]
                        pbf = PowerupBoxFactory.get()
                        self._wi.append(
                            PowerupBox(
                                position=position,
                                poweruptype=(
                                    pbf.get_random_powerup_type(
                                        excludetypes=['curse', 'wonder']
                                    )
                                ),
                            ).autoretain().node
                        )
...
class Bomb(bs.Actor):
    ...
    def __init__(
        ...
        bomb_type can be 'ice','impact','land_mine','normal','sticky',
        'portal', 'dev', 'impulse', 'wonder, or 'tnt'. Note that for impact,
        icepact, land_mine, portal, or dev bombs you have to call arm() before
        they will go off.
        """
        super().__init__()
        ...
        self.scale = bomb_scale * factory.bomb_scale_mult
        ...
        if self.bomb_type in ('impact', 'icepact', 'portal', 'dev'):
        ...
        self.density = density * factory.density_mult
        ...
        if self.bomb_type == 'land_mine':
            ...
            self.node = bs.newnode(
                ...
                attrs={
                    'position': position,
                    'velocity': velocity,
                    'mesh': factory.impulse_bomb_mesh,
                    'body': 'box',
                    'body_scale': self.scale,
                    'mesh_scale': self.scale,
                    'shadow_size': 0.3,
            ...
            self.light = bs.newnode(
                'light', attrs={'height_attenuated': False, 'color': (1, 1, 1)}
            )
            ...
            bs.animate(self.light, 'intensity', {0: 1, 3: 5})
            ...
            self.node = bs.newnode(
                ...
                attrs={
                    ...
                    'mesh': factory.impact_bomb_mesh,
                    'shadow_size': 0.3,
                    'color_texture': factory.portal_tex,
                    'reflection': 'powerup',
                    'reflection_scale': [1.5],
                    'materials': materials,
                    'gravity_scale': gravity_scale,
            ...
            s = bs.newnode(
                ...
                owner=self.node,
                attrs={
                    'color': [c * 2 for c in self.owner.color],
                    'radius': self.scale / 2,
                },
            )
            self.node.connectattr('position', s, 'position')

        elif self.bomb_type == 'dev':
            fuse_time = None
            self.node = bs.newnode(
                'prop',
                delegate=self,
                attrs={
                    ...
                    'body': 'sphere',
                    'body_scale': self.scale,
                    'mesh_scale': self.scale / 4,
                    'mesh': factory.dev_mesh,
                    'color_texture': factory.dev_tex,
                    'reflection': 'powerup',
                    'reflection_scale': [1.5],
                    'materials': materials,
                    'gravity_scale': gravity_scale,
            ...
            s = bs.newnode(
                'shield',
                owner=self.node,
                attrs={'color': (5, 5, 5), 'radius': self.scale * 0.6},
            )
            self.node.connectattr('position', s, 'position')
            ...
            elif self.bomb_type == 'wonder':
                tex = factory.wonder_tex
            else:
            ...
            bs.animate(
                self.node, 'fuse_length', {0.0: 1.0, fuse_time: 0.0},
            )

        # Light the fuse!!!
        if self.bomb_type not in ('land_mine', 'tnt', 'portal', 'dev'):
        ...
        if anim:
            if self.bomb_type == 'dev':
                bs.animate(
                    self.node,
                    'mesh_scale',
                    {0: 0, 0.2: 0.325 * self.scale, 0.26: self.scale / 4},
                )
            else:
                bs.animate(
                    self.node,
    ...
    def _handle_impact(self) -> None:
        ...
        if node:
            if self.bomb_type in ('impact', 'icepact', 'portal', 'dev') and (
                node is self.owner
                or (
                    isinstance(node_delegate, Bomb)
                    and node_delegate.bomb_type
                    in ('impact', 'icepact', 'portal', 'dev')
    ...
    def explode(self) -> None:
        ...
        self._exploded = True
        if self.node:
            if self.bomb_type == 'portal':
                from bascenev1lib.actor.anomalies import Portal
                ...
                p0 = Portal(
                    ...
                    self.scale,
                    True,
                    bs.existing(self._source_player),
                    color,
                ).autoretain()
                p1 = Portal(
                    self.node.position,
                    ...
                    True,
                    bs.existing(self._source_player),
                    (2.5 - color[0], 2.5 - color[1], 2.5 - color[2]),
                    p0,
                ).autoretain()
                p0.pair = p1
                bs.timer(20, bs.WeakCall(p0.handlemessage, bs.DieMessage()))
                bs.timer(20, bs.WeakCall(p1.handlemessage, bs.DieMessage()))
            elif self.bomb_type == 'dev':
    ...
    def arm(self) -> None:
        ...
        if not self.node:
            return
        factory = BombFactory.get()
        intex: Sequence[bs.Texture] | None = None
        if self.bomb_type == 'land_mine':
        ...
        elif self.bomb_type in ('icepact', 'portal', 'dev'):
            bs.timer(
                0.25,
                bs.WeakCall(
                    self._add_material, factory.land_mine_blast_material
            ...
            raise RuntimeError(
                'arm() should only be called on land-mines, impact/icepact '
                'bombs or portal/dev modules'
            )
        if intex:
    ...
    def _handle_hit(self, msg: bs.HitMessage) -> None:
        ...
        if not self._exploded and (
            not ispunched
            or self.bomb_type
            in ['impact', 'land_mine', 'icepact', 'portal', 'dev']
        ):
