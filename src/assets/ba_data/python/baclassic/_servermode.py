...
class ServerController:
    ...
    def _launch_server_session(self) -> None:
        ...
        bascenev1.set_public_party_name(
            ('␞␟' if self._config.overclocked_recognition else '')
            + self._config.party_name
        )
        ...
        appcfg['Vanillaclocked'] = self._config.vanillaclocked
        if self._config.config_override:
            for k, v in self._config.config_override.items():
                appcfg[k] = v
        appcfg.apply()
