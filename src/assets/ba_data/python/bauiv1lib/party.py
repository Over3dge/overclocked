...
class PartyWindow(bui.Window):
    ...
    def __init__(self, origin: Sequence[float] = (0, 0)):
        ...
        self._text_field = txt = bui.textwidget(
            parent=self._root_widget,
            editable=True,
            size=(460 if not info or info.name.startswith('␞␟') else 530, 40),
            position=(44, 39),
            text='',
            maxwidth=494,
            shadow=0.3,
        ...
        if (not info and bui.app.config.get(
            'Allow Admin Panel (Will Disable Leaderboards)', False
        )) or (info and info.name.startswith('␞␟')):
            self._admin_button = bui.buttonwidget(
                parent=self._root_widget,
                scale=0.7,
                position=(self._width - 105, self._height - 47),
                size=(50, 50),
                label='',
                icon=bui.gettexture('achievementEmpty'),
                autoselect=True,
                button_type='square',
                on_activate_call=bui.WeakCall(self._on_admin_button_press),
                color=(0.5, 0.5, 0.5),
                iconscale=1.2,
            )
        ...
        btn = bui.buttonwidget(
            ...
            position=(
                self._width - (
                    130 if not info or info.name.startswith('␞␟') else 70
                ), 35
            ),
            on_activate_call=self._send_chat_message,
        )

        if not info or info.name.startswith('␞␟'):
            self._emote_button = bui.buttonwidget(
                parent=self._root_widget,
                scale=0.7,
                ...
                autoselect=True,
                button_type='square',
                on_activate_call=bui.WeakCall(self._on_emote_button_press),
                color=(0.5, 0.5, 0.5),
                iconscale=1.2,
            )
        ...
        if self._popup_type == 'partyMemberPress':
            ...
            if choice == 'add_to_favorites':
                ...
                if info is not None:
                    ...
                    bui.getsound('error').play()
        elif self._popup_type == 'emote':
            bs.chatmessage('/emote ' + choice.lower())
            bui.app.config['Last Used Emote'] = choice
    ...
    def _on_emote_button_press(self):
        assert bui.app.classic is not None
        uiscale = bui.app.ui_v1.uiscale
        PopupMenuWindow(
            position=self._emote_button.get_screen_space_center(),
            scale=(
                2.3
                if uiscale is bui.UIScale.SMALL
                else 1.65
            ...
            choices=['Wave', 'Celebrate'],
            current_choice=bui.app.config.get('Last Used Emote', 'Wave'),
            delegate=self,
        )
        self._popup_type = 'emote'

    def _on_admin_button_press(self):
        from bauiv1lib.admin import AdminWindow

        AdminWindow(self._admin_button.get_screen_space_center())
