...
class PublicGatherTab(GatherTab):
    ...
    def _build_join_tab(
        ...
        bui.app.config.setdefault('Only Show OVERCLOCKED Parties', False)
        bui.app.config.apply_and_commit()
        self._show_oc = ConfigCheckBox(
            parent=self._container,
            configkey='Only Show OVERCLOCKED Parties',
            position=(50, v - 50),
            size=(c_width - 100, 30),
            scale=1.0,
        )
    ...
    def _update_party_lists(self) -> None:
        ...
        if (
            ...
            if bui.app.config.get('Only Show OVERCLOCKED Parties', False):
                self._parties_displayed = {
                    k: v
                    for k, v in self._parties_displayed.items()
