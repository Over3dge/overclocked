...
class AllSettingsWindow(bui.Window):
    ...
    def __init__(
        ...
        width = 1000 if uiscale is bui.UIScale.SMALL else 805
        height = 435
        ...
        baseh = 170
        x_offs = (
            (105 if uiscale is bui.UIScale.SMALL else 72) - basew
        )  # now unused
        x_offs2 = x_offs + basew - 7
        x_offs3 = x_offs + 2 * (basew - 7)
        x_offs4 = x_offs + 3 * (basew - 7)
        x_offs5 = x_offs + 1.5 * (basew - 7)
        x_offs6 = x_offs + 2.5 * (basew - 7)
        ...
        ocb = self._overclocked_button = bui.buttonwidget(
            parent=self._root_widget,
            autoselect=True,
            position=(x_offs5, v),
            size=(basew, baseh),
            button_type='square',
            label='',
            on_activate_call=self._do_overclocked,
        )
        _b_title(x_offs5, v, ocb, 'Overclocked Settings')
        imgw = imgh = 120
        bui.imagewidget(
            parent=self._root_widget,
            ...
            size=(imgw, imgh),
            color=(0.8, 0.95, 1),
            texture=bui.gettexture('overclockedLogo'),
            draw_controller=ocb,
        )
        ...
        _b_title(x_offs6, v, avb, bui.Lstr(resource=self._r + '.advancedText'))
        imgw = imgh = 120
        bui.imagewidget(
            parent=self._root_widget,
    ...
    def _preload_modules() -> None:
        ...
        import bauiv1lib.settings.overclocked as _unused5
        import bauiv1lib.settings.advanced as _unused6
    ...
    def _do_overclocked(self) -> None:
        # pylint: disable=cyclic-import
        from bauiv1lib.settings.overclocked import OverclockedSettingsWindow
        ...
        assert bui.app.classic is not None
        bui.app.ui_v1.set_main_menu_window(
            OverclockedSettingsWindow(
                origin_widget=self._overclocked_button
            ).get_root_widget(),
            from_window=self._root_widget,
        )
    ...
    def _save_state(self) -> None:
        try:
            ...
            if sel == self._controllers_button:
                ...
                sel_name = 'Audio'
            elif sel == self._overclocked_button:
                sel_name = 'Overclocked'
    ...
    def _restore_state(self) -> None:
        try:
            ...
            elif sel_name == 'Overclocked':
                sel = self._overclocked_button
